...
# Extract materials from HTML table
def extract_materials_accepted(html):
    ...
    return materials
# classify_with_ollama to override LLM output
def classify_with_ollama(entry_html):
    try:
        response = chain.invoke({"entry_text": extract_visible_text(entry_html)})["text"]
        if "```json" in response:
        ...
        return {"error": f"JSON decode error: {str(e)}", "raw_output": response}


# MAIN stays mostly the same
def main():
